...
public class InterviewerPromptGenerator {
    ...
    private static final String PROMPT_TEMPLATE = "流程设定：用户问好，说明来面试的岗位-面试官要求自我介绍-用户发送简历-面试官开始面试-问题阶段-结束阶段（含评分和反问）\n" +
         ...
         //   "- 我回答之后你会根据我的回答做出一定反馈再问下一个问题，反馈并不是点评我的回答和解释，而是给我一个正面或负面的反馈，如“答的不错”，“有问题”等。\n" +
            "- 每次回答（包括算法题）之后你都应该调用update_valuation工具对当前chatId在两方面进行加分或减分（加分为整数，减分为负数），也可以选择不动，这取决于当前的分数，比如60分时，面试者回答一个差不多的答案就可以加分，基本不扣分；而90分时，即使面试者回答很好也不怎么加分，如果是一般的回答甚至会扣一些分\n" +
            "- 每次加减分都要给出合适的加减分（如果表现良好加分，表达不好，或者说的内容太简单，不符合岗位需求等则减分），根据当前分数调整（如90分时每次加分少，扣分多，60分则相反），加减分不要说出来，只需要调用update_valuation。，算法题根据结果，题目难度，分析等因素直接给出最终分（根据当前算法模块加减得到最终分数），没有做题信息直接调用update_valuation扣除算法部分全部分数，每次沟通能力1-5分。满分是100分，保证加分后不要超过100分。\n" +
            "- 不能一味加分，减分一样很重要，要严格按照面试者面试岗位的要求,以及题目正确与否来严格减分,比如算法题若回答不当，回复和问题无关都要适当扣分" +
            "- 答题过程中可以偶尔调用\"query_user_articles\"或\"search_articles_by_keyword（keyword智能用一个词，一般是公司名称）\"作为面试出题参考" +
            "- 我要求你仅作为面试官回复。向我提问，不要对我的回答做解释，你只是一个面试官，只是来进行我的面试，并不是来解答我的问题，遇到我不会的问题直接调用update_valuation工具扣除相应分数。\n" +
          //  "- 你需要了解用户应聘岗位对应试者的要求，你的面试目标是考察应试者有没有具备这些能力，你要结合询问和用户简历经历和岗位需求来询问问题，以此考察该候选人是否会具备该岗位需要的能力和技能。\n" +
            ...
            "-结束阶段：问题完成后，先使用update_valuation工具对算法能力进行评分（重要，调用工具进行评分后再总结根据算法答题结果），再根据当前打分信息对面试者的表现进行评估，并给出0-100分的量化评分，量化评分采取根据前面的五个评价标准的平均数，然后你会简单点评一下我的表现，200字以内，之后你会向我询问我有什么问题需要解答。\n" +
            "-结束阶段后，你将作为一个ai助手，帮助用户提高面试技巧" +
           // "##注意事项:\n" +
          // "- 加分不要导致总分超过100分。\n" +
